...
import {
  ...
  DASHBOARD_MEDS_HASH,
  NOTIFICATION_DROPDOWN_MAX_ITEMS,
...
export default function PatientMedicationNotificationsBell({
  ...
  const dropdownRows = useMemo(() => {
    const apiPart = apiItems.map((n) => ({
      kind: "api",
      n,
      ts: new Date(n.createdAt || 0).getTime(),
    }));
    const medPart = missed.map((row) => ({
      kind: "med",
      row,
      ts: now.getTime() - (row.minutesPast || 0) * 60_000,
    }));
    return [...apiPart, ...medPart].sort((a, b) => b.ts - a.ts).slice(0, NOTIFICATION_DROPDOWN_MAX_ITEMS);
  }, [apiItems, missed, now]);
